...
def train_or_eval_model(args, model, loss_func, dataloader, epoch, optimizer=None, train=False, test=False):
    vidnames = []
    preds=[]
    labels=[]
    losses = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feat_type', type=str, default='utt', help='feature type [utt, frm_align, frm_unalign]')
    parser.add_argument('--feat_scale', type=int, default=None,
    ...
    parser.add_argument('--lr', type=float, default=None, metavar='lr',
                        help='set lr rate')  # 如果是None, lr 作为了 hyper-params 通过 model-tune.yaml 调节
    ...
    # logging.basicConfig(filename=os.path.join(log_path, log_filename), level=logging.INFO,
    ...
    for ii in range(len(train_loaders)):
        ...
        loss = get_loss_func(args.loss_name)
        ...
        for epoch in range(args.epochs):
            ...
            train_results = train_or_eval_model(args, model, loss, train_loader, epoch=epoch,
                                                optimizer=optimizer, train=True)
            eval_results = train_or_eval_model(args, model, loss, eval_loader, epoch=epoch,
                                               optimizer=None, train=False)
            func_update_storage(inputs=eval_results, prefix='eval', outputs=epoch_store)

            ## use args.metric_list to determine best_index
            train_metric = gain_metric_from_results(train_results, args.metric_list)
            eval_metric = gain_metric_from_results(eval_results, args.metric_list)
            whole_metrics.append(eval_metric)

            print(f'Epoch: {epoch + 1}')
            for metric in args.metric_list:
                print(
                    f'Metric: {metric}; Train results: {train_metric[metric]:.4f}; Eval results: {eval_metric[metric]:.4f}')

            # logging.info('epoch:%d; metric:%s; train results:%.4f; eval results:%.4f' % (
            # epoch + 1, args.metric_list, train_metric, eval_metric))
            # testing and saving
            for jj, test_loader in enumerate(test_loaders):
                test_results = train_or_eval_model(args, model, loss, test_loader, epoch=epoch,
                                                   optimizer=None, train=False, test=True)
