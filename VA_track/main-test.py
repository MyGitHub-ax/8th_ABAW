...
def output_txt(prediction_dict, save_path):
    pseudo_label_file = '/data/yanglongjiang/Data/ABAW/Aff-Wild2/predictions.txt'
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, epoch, optimizer=None, train=False,test=False):
    ...
    for iter, data in enumerate(dataloader):
        ...
        features, vals_out, interloss = model(batch)
        if test:
    ...
    # results, _ = dataloader.calculate_results(emo_probs, emo_labels, val_preds, val_labels)
    save_results = dict(
        preds=val_preds,
        labels=val_labels,
        names=vidnames,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', action='store_true', default=True, help='whether to save model, default: False')
    ...
    log_path='/data/yanglongjiang/Code/ABAW6th/MERBench/log'
    ...
    # 确保模型保存根目录存在
    os.makedirs(save_modelroot, exist_ok=True)
    for ii in range(len(train_loaders)):
        ...
        # 初始化最佳指标和模型状态
        best_ccc = -np.inf  # CCC 越高越好，初始值设为负无穷
        ...
        for epoch in range(args.epochs):
            ...
            whole_metrics.append(eval_metric['ccc'])
            print ('epoch:%d; metric:%s; train results:%.4f; eval results:%.4f' %(epoch+1, args.metric_name, train_metric['ccc'], eval_metric['ccc']))
            logging.info('epoch:%d; metric:%s; train results:%.4f; eval results:%.4f' %(epoch+1, args.metric_name, train_metric['ccc'], eval_metric['ccc']))
            ## 更新最佳模型状态
            current_ccc = eval_metric['ccc']
        ...
        if best_model_state is not None:
            # 生成保存路径（示例：./saved_models/fold1_ccc0.8523.pth）
            save_path = os.path.join(
                save_modelroot,
                f"fold{ii + 1}_ccc{best_ccc:.4f}.pth"
            )
            torch.save(best_model_state, save_path)
            print(f"Fold {ii + 1} 最佳模型已保存至: {save_path}")
        else:
            print(f"Fold {ii + 1} 未保存模型（无有效 CCC 值）")
        # clear memory
        del model
        del optimizer
    ...
    save_path = f'{save_resroot}/cv_{prefix_name}_{formatted_time}.npz'
