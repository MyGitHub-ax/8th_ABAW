...
sys.path.append('/data/yanglongjiang/project/ABAW')
import config
...
def func_read_frames(face_dir, vid):
    """从npy文件加载视频帧（最小修改版）"""
    # 新路径：每个视频对应一个npy文件
    npy_path = os.path.join(face_dir, vid,"video128.npy")
    ...
    # frames = []
    # print('reading img from ', os.path.join(face_dir, vid))
    # img_list = sorted(os.listdir(os.path.join(face_dir, vid)))  # 获取所有图像并排序
    # # 使用 tqdm 添加进度条
    # for i in tqdm.tqdm(range(0, len(img_list), 15), desc='Reading Frames', unit='frame'):  # 每隔15个图像读取一个
    #     img = img_list[i]
    #     if not img.endswith('.jpg'):
    #         continue
    #     img_path = os.path.join(face_dir, vid, img)
...
if __name__ == '__main__':
    ...
    print(f'==> Extracting {args.model_name} embeddings...')
    model_name = args.model_name.split('.')[0]
    face_dir = "/data/emotion-data/abaw/PreProcessed/RJCMA/compacted_48"
    # face_dir = config.PATH_TO_FACE[args.dataset]
    ...
    # load model
    if args.model_name in [CLIP_VIT_BASE, CLIP_VIT_LARGE, DATA2VEC_VISUAL, VIDEOMAE_BASE,
                           VIDEOMAE_LARGE]:  # from huggingface
        model_dir = os.path.join(config.PATH_TO_PRETRAINED_MODELS, f'transformers/{args.model_name}')
        model = AutoModel.from_pretrained(model_dir)
        processor = AutoFeatureExtractor.from_pretrained(model_dir)
    elif args.model_name in [DINO2_LARGE, DINO2_GIANT]:
        model_dir = os.path.join(config.PATH_TO_PRETRAINED_MODELS, f'transformers/{args.model_name}')
        model = AutoModel.from_pretrained(model_dir)
        processor = AutoImageProcessor.from_pretrained(model_dir)
    elif args.model_name in [EVACLIP_VIT]:  # from timm
        model_path = os.path.join(config.PATH_TO_PRETRAINED_MODELS, f'timm/{args.model_name}/model.safetensors')
    ...
    elif args.model_name in [DINO_VITB8]:
        model_dir = os.path.join(config.PATH_TO_PRETRAINED_MODELS, f'new/{args.model_name}')
    ...
    for i, vid in enumerate(vids, 1):
        ...
        with torch.no_grad():
            ...
            if args.model_name in [CLIP_VIT_BASE, CLIP_VIT_LARGE]:
                ...
                for batch in batches:
                    if args.gpu != -1: batch = batch.to("cuda")
            ...
            elif args.model_name in [DATA2VEC_VISUAL]:
                frames = [func_opencv_to_image(frame) for frame in frames]
            ...
            elif args.model_name in [VIDEOMAE_BASE, VIDEOMAE_LARGE]:
                ...
                for batch in batches:
                    ...
                    if args.gpu != -1: inputs = inputs.to("cuda")
            ...
            elif args.model_name in [EVACLIP_VIT]:
                frames = [func_opencv_to_image(frame) for frame in frames]
