...
def simplify_csv():
    """这段 Python 代码定义了一个名为 simplify_csv 的函数，其主要功能是处理 MELD 数据集的 CSV 文件，
    将其简化为包含视频 ID、情感标签和话语文本的文本文件，并进行数据清理和错误检查，同时记录处理过程中的日志信息。"""
...
def process_one_video(p_video, out_dir: str, store_top_n_faces: int,
    ...
    """
      该函数用于处理单个视频文件，进行人脸检测、裁剪和对齐，并将结果保存到指定目录。
      :param p_video: 视频文件的路径
      :param out_dir: 输出人脸图像的目录
      :param store_top_n_faces: 每个帧要存储的最多人脸数量
      :param log_file: 用于记录视频处理信息的日志文件对象
      :param nblocks: 数据块的总数
      :param process_block: 当前处理的数据块编号
      :param ds: 数据集名称
      :param split: 数据划分（如训练集、验证集）
      :param frame_log_file: 用于记录帧级处理信息的日志文件对象
      """
...
def crop_faces_align(ds: str, split: str, nblocks: int, process_block: int):
    ...
    # 拼接数据划分文件的路径，该文件记录了属于当前划分的视频 ID
    ...
    #log-nblocks-8-process-block-0.txt
    frame_log_file_path = join(_dir_frame_log_faces,
                               f'frame-log-nblocks-{nblocks}-'
                               f'process-block-{process_block}.txt')
    # 以写入模式打开人脸裁剪和对齐日志文件log-nblocks-8-process-block-0.txt
...
def read_annotation(path_annot: str, video_path) -> list:
    ...
    with open(path_annot, 'r', encoding="UTF-8") as f:
        ...
        for row in csv_reader:
            ...
            #video_path:视频路径,_delta:时长,label_str:标签
...
def truncate_one_video(video_path: str, annotation_path: str,
    ...
    """其主要功能是根据给定的视频文件路径、注释文件路径和输出目录，将视频按照注释文件中的时间戳进行裁剪，
    生成多个子视频片段，并将这些片段分类保存到输出目录下以标签命名的子目录中。同时，代码会检查视频文件是否损坏，
    并记录相关日志信息"""
    ...
    for seg in timestamps:
        ...
        cmd = f"ffmpeg -y -ss {_start} -i {video_path} -to {_end} -c:v copy -c:a copy {dest_video}"
...
def truncate_all_videos():
    ...
    videos_folders = join(videos_folders, 'videos')
    print("videos_folders",videos_folders)
...
def dump_split(data: list, dest_path: str, trimmed_dir_path: str,
               transcript: dict):
    """这段代码定义了一个 dump_split 函数，它的功能是将数据（包括视频的标签和转录文本）写入一个 CSV 文件中，
    并将标签转化为对应的整数值。"""
...
def split_data(transcript: dict):
    """按照交叉验证的方式进行划分，生成多个训练集、验证集和测试集的划分组合，并将这些划分信息保存到文件中，
    最后将划分结果复制到指定的根目录下"""
    ds = constants.C_EXPR_DB
    baseurl = get_root_wsol_dataset()
    videos_folders = join(baseurl, ds, 'videos')
...
def load_transcript() -> dict:
    """
       该函数的作用是从指定的 CSV 文件中加载转录数据，并将其存储在一个字典中。
       返回值为一个字典，字典的键是数据的 ID，值是对应的转录文本。
    """
    ds = constants.C_EXPR_DB
    baseurl = get_root_wsol_dataset()
...
if __name__ == "__main__":
    ...
    assert split in [constants.TRAINSET, constants.VALIDSET], split
    #truncate_all_videos()
    """truncate_one_video("/data/liuran/lr/real/feature-vs-text-compound-emotion-main/dataset/C-EXPR-DB/videos/videos",
                       "/data/liuran/lr/real/feature-vs-text-compound-emotion-main/dataset/annotation",
                       "/data/liuran/lr/real/feature-vs-text-compound-emotion-main/dataset/C-EXPR-DB/videos")"""
