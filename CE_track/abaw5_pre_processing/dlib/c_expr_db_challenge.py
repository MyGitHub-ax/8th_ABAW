...
def crop_faces_align(ds: str, split: str, nblocks: int, process_block: int):
    ...
    assert split in [constants.TESTSET], split  # train == valid == test
...
def pull_stats(t_stamps: list, log_file):
    """对视频剪辑的统计信息进行汇总和记录。它接收一个包含视频剪辑信息的列表 t_stamps 和一个日志文件对象 log_file 作为参数，
    然后计算每个类别（标签）的剪辑数量、总时长，并输出所有剪辑的总数和总时长，将这些统计信息记录到日志文件中"""
    stats = {}
...
def get_list_clips_per_cl(clips_dir: str) -> dict:
    """函数用于获取指定目录下每个子文件夹中的 .mp4 视频剪辑文件列表，并将这些文件列表按子文件夹名称（类别）存储在一个字典中。
    同时，对每个类别下的文件列表进行 100 次随机打乱操作，以增加随机性。"""
    out = dict()
...
def create_folders(data: dict, n: int) -> dict:
    """该函数用于将每个类别下的文件列表进一步划分成 n 个折叠（folds），并将每个折叠中的文件合并成一个列表，
    最终返回一个包含 n 个折叠的字典"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ds", type=str, default=constants.C_EXPR_DB_CHALLENGE,
                        help="dataset name.")
    parser.add_argument("--split", type=str, default=constants.TESTSET,
