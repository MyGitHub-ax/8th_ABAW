...
def gpu_memory_stats(device: int) -> str:
    ...
    try:
        ...
        # 分割每一行输出，获取每个 GPU 的内存信息
        lines = result.stdout.strip().split('\n')
        if device < len(lines):
            # 解析指定 GPU 的内存信息
            free_memory, total_memory = map(int, lines[device].split(','))
            used_memory = total_memory - free_memory
            # 转换内存单位为 GB
            unit = 'GB'
            norm = 1024
            # 格式化内存使用信息
            msg = f'GPU {device} MEM: USED {used_memory / norm}'
            msg += f" Free: {free_memory / norm}"
            msg += f" Total: {total_memory / norm}"
            msg += f' {unit}.'
            return msg
        else:
            print(f"Error: GPU device {device} not found.")
            return None
    except (subprocess.CalledProcessError, AssertionError, ValueError) as e:
        print(f"Error: {e}")
        return None
