...
class GenericVideoPreprocessing(object):
    def __init__(self, part, config, cnn_path=None):
        ...
        self.dataset_info = self.init_dataset_info()#字典dict_keys(['trial', 'subject_no', 'trial_no', 'length', 'partition', 'trial_list', 'pseudo_partition', 'data_folder'])
    ...
    def prepare_data(self, logfile=None):
        """主要用于数据预处理。它根据对象的属性，决定是否加载和初始化不同的模型（如 VGGish、标点模型、BERT 等），
        然后遍历选定的视频数据，对每个视频进行一系列的处理，包括视频处理（如裁剪、帧率调整、特征提取等）、
        音频处理（如转换为 WAV、特征提取等）、文本处理（如转录、添加标点、词嵌入提取等）以及 EEG 处理。
        最后，将处理记录保存为 pickle 文件，并生成数据集信息。"""
