...
class Experiment(GenericExperiment):
    ...
    def load_dataset_info(self) -> dict:
        """根据给定的数据集名称 (self.args.dataset_name)，加载与该数据集相关的训练、验证和测试集的元数据
        （通常是存储在 pickle 文件中的信息），并将其以字典的形式返回"""
        ...
        if ds == constants.MELD:
            ...
            dataset_info[constants.VALIDSET] = copy.deepcopy(
                dataset_info[constants.TESTSET])
            dataset_info[constants.TRAINSET] = copy.deepcopy(
                dataset_info[constants.TESTSET])
    ...
    def init_dataset(self, data, continuous_label_dim, mode, fold=None):
        ...
        """for batch in dataset:
            print("------------------------------------")
            print(type(batch[0]['video'].shape))
            print("------------------------------------")"""
        return dataset
    ...
    def get_selected_continuous_label_dim(self):
        """主要功能是根据数据集名称 ds 和情感类型 self.emotion 来确定要选择的连续标签维度"""
