...
class Trainer(GenericVideoTrainer):
    ...
    def inference(self, dataloader):
        """用于在验证集或测试集上生成预测结果，并评估模型性能。核心功能包括：

    推理模式配置：禁用随机性保证结果可复现，设置模型为评估模式。

    数据批量处理：遍历数据加载器，处理输入数据与标签。

    前向推理：支持常规推理和窗口化推理（处理长序列）。

    结果收集：保存每个视频的帧级预测结果（logits）和真实标签。

    性能评估与保存：计算指标（如准确率），并持久化预测结果。"""
        self.default_random()
        ...
        for batch_idx, (X, trials, lengths, indices) in tqdm.tqdm(
            ...
            with autocast(enabled=self.args.amp):
                ...
                per_video_frame_logits[_v_id] = {
                    ...
                    #'trials':trials.cpu().numpy().flatten()
                }
    ...
    def compute_perf(self, data) -> dict:
        ...
        """根据输入的预测数据 data 计算多个性能指标，这些指标包括宏 F1 分数（Macro F1）、
        加权 F1 分数（Weighted F1）、分类准确率（Class Accuracy）和混淆矩阵（Confusion Matrix），
        并且分别在帧级别（frame level）和视频级别（video level）进行计算。
        同时，该方法会根据不同的忽略类别（ignore_class）进行多次计算，最终返回一个包含所有计算结果的字典。"""
        perf = dict()
    ...
    def optimize(self,
        ...
        if self.best_epoch_info is None:
            print(self.best_epoch_info+"不是None的")
            self.best_epoch_info = {
                'model_weights': copy.deepcopy(self.model.state_dict()),
                'loss': 1e10,
                'ccc': -1e10
        ...
        for epoch in tqdm.tqdm(np.arange(0, self.max_epoch), ncols=80,
            ...
            #epochpath = '/data/liuran/feature-vs-text-compound-emotion-main/exps/MELD/fold-0/id_03_09_2025_16_20_48_088070__8018213'
